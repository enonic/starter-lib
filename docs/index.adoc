...
To add assets or schema files to a library simply add the files with the same path as they would be used in an application.
...
To create a new JavaScript library simply create a file with extension ``.js`` inside the project directory with path ``src/main/resources/lib/<name>.js``
